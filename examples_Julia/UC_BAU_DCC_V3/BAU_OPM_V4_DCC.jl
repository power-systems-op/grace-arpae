...
## Initilization of the next UC Run
    ...
    # Setting initial values for BUCR1 (next hour), SUCR1, and BUCR2
        for g=1:N_Gens
            ...
            global BUCR1_Init_genOnOff[g] = round(Int, JuMP.value.(BUCR1_genOnOff[g]));
            global BUCR1_Init_genOut[g] = round(Int, JuMP.value.(BUCR1_genOut[g]));
            if h==(INITIAL_HR_SUCR-INITIAL_HR_FUCR)
                global BUCR2_Init_genOnOff[g] = round(Int, JuMP.value.(BUCR1_genOnOff[g]));
                global BUCR2_Init_genOnOff[g] = round(Int, JuMP.value.(BUCR1_genOnOff[g]));
                global BUCR2_Init_genOut[g] = round(Int, JuMP.value.(BUCR1_genOut[g]));
                global SUCR_Init_genOnOff[g] = round(Int, JuMP.value.(BUCR1_genOnOff[g]));
                global SUCR_Init_genOut[g] = round(Int, JuMP.value.(BUCR1_genOut[g]));
            end
        end
        for p=1:N_StorgUs
...
#TODO: Delete this section
        ...
        # Write the conventional generators' schedules
        #XLSX.openxlsx(".\\outputs\\GenOutputs.xlsx", mode="w") do xf
        XLSX.openxlsx(".\\outputs\\BUCR2_GenOutputs.xlsx", mode="rw") do xf
            sheet = xf[1]
            #XLSX.rename!(sheet, "new_sheet")
            #sheet["A1:I1"] = ["Hour" "GeneratorID" "VariableCost" "MinPowerOut" "MaxPowerOut" "Output" "On/off" "ShutDown" "Startup"]
            for g=1:N_Gens
        ...
        # Writing storage units' optimal schedules into spreadsheets
        #XLSX.openxlsx(".\\outputs\\StorageOutputs.xlsx", mode="w") do xf
        XLSX.openxlsx(".\\outputs\\BUCR2_StorageOutputs.xlsx", mode="rw") do xf
            sheet = xf[1]
            #XLSX.rename!(sheet, "new_sheet")
            for p=1:N_StorgUs
        ...
        # Writeing the transmission flow schedules into spreadsheets
        #XLSX.openxlsx(".\\outputs\\TranFlowOutputs.xlsx", mode="w") do tf
        XLSX.openxlsx(".\\outputs\\BUCR2_TranFlowOutputs.xlsx", mode="rw") do xf
            sheet = xf[1]
                #XLSX.rename!(sheet, "new_sheet")
                for n=1:N_Zones, m=1:M_Zones
