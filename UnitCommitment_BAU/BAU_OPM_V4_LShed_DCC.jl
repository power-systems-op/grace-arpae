"""
This algorithm evaluates and solves  the Business as Usual Unit Commitment
Model of Duke's Energy Sytem
...
...
# Arguments
- `filePath::String`: the path where the CSV files with the system information.
...

# Examples
```julia-repl
julia> solveUC("Input Folder")
...
# Generators location map: if a generator g is located in zone z Map_Gens[g,z]=1;
# and 0 otherwise
...
# Selecting data from input files
...
for day = INITIAL_DAY:FINAL_DAY
    ...
    @info "FUCR Obj. value for day ", day, " is ", JuMP.objective_value(FUCRmodel)
    println("------------------------------------")
    ...
    @info("Data from generators and Demand downloaded correctly")
    println("---------------------------")
    ...
    for h=1:INITIAL_HR_SUCR-INITIAL_HR_FUCR # number of BUCR periods in between FUCR and SUCR
        ...
        @info "BAUC1 Obj. value for day ", day, " and hour ", h+INITIAL_HR_SUCR, " is: ", JuMP.objective_value(BUCR1model)
        println("------------------------------------")
    ...
    #Initialization of commitment and dispatch variables at t=0 (representing
    #the last hour of previous scheduling horizon day=day-1 and t=24)
    ...
    ## This block models the second Balancing Unit Commitment Run, which is for
    #the time range between the morning and evening UC Runs
    for h=1:24-INITIAL_HR_SUCR+INITIAL_HR_FUCR # number of BUCR periods in between FUCR and SUCR
        ...
        @info "BAUC2 Obj. value for day ", day, " and hour ", h+INITIAL_HR_SUCR, " is: ", JuMP.objective_value(BUCR2model)
        println("------------------------------------")
