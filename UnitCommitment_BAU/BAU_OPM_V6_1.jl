...
const N_Zones = 2 # number of tranmission zones
const M_Zones = 2 # number of tranmission zones
const N_Blocks =7 # number of heatrate blocks/segments in the incremental heat rate curves
# INITIAL_DAY and Final_day identify the time period for which the model simulates the grid operations
const INITIAL_DAY = 53
const FINAL_DAY = 55  # in this case, Final-Day=2 means the model simulates first and second day of the year
...
const DemandCurt_C = 3000; # Load-Shedding Penalty $/MWh
const OverGen_C = 4000; # Over-generation Penalty $/MWh
const DemandCurt_Max = 100; # Load-Shedding maximum (MWh)
const OverGen_Max = 100; # Over-generation maximum in MW (MWh)

const ViolPenalty = 500; #penalty for violating minimum and maximum generation limit
const ViolMax = 10; # maximum violation allowed in min and max generation limit constraints, which is not enforced in the current vcersion of the model
...
# Slow-Start Generators' specifications
DF_Generators = CSV.read(".//inputs//csv//data_generators.csv", DataFrame);
# Slow-Start Generators location map: if a generator g is located in zone z Map_Gens[g,z]=1; and 0 otherwise
...
FUCR_Demands = readdlm(".//inputs//csv//data_demand.csv", ','; header = true); # FUCR-Demand represents demand forecast at 6 am
SUCR_Demands = readdlm(".//inputs//csv//data_demand_updated.csv", ','; header = true); # SUCR_Demand represents demand forecast at 5 pm (17)
BUCR_Demands = readdlm(".//inputs//csv//data_demand_actual.csv", ','; header = true); # BUCR_Demand represents hourly-average real-time demand data

# solar generation data at each location
FUCR_SolarGs = readdlm(".//inputs//csv//data_solar.csv", ','; header = true); # solar forecast at 6 am
SUCR_SolarGs = readdlm(".//inputs//csv//data_solar_updated.csv", ','; header = true); # solar forecast at 5 pm
BUCR_SolarGs = readdlm(".//inputs//csv//data_solar_actual.csv", ','; header = true);# solar generation in real-time

# wind energy data for each location
FUCR_WindGs = readdlm(".//inputs//csv//data_wind.csv", ','; header = true); # wind forecast at 6 am
SUCR_WindGs = readdlm(".//inputs//csv//data_wind_updated.csv", ','; header = true); # wind forecast at 5 pm
BUCR_WindGs = readdlm(".//inputs//csv//data_wind_actual.csv", ','; header = true); # wind forecast in real-time

#hydro generation data for each location
FUCR_HydroGs = readdlm(".//inputs//csv//data_hydro.csv", ','; header = true);# at 6 am
SUCR_HydroGs = readdlm(".//inputs//csv//data_hydro_updated.csv", ','; header = true); # at 5 pm
BUCR_HydroGs = readdlm(".//inputs//csv//data_hydro_actual.csv", ','; header = true); # in real time

TranC = readdlm(".//inputs//csv//LineCapacity.csv", ','; header = true); # Transmission capacity between every two zones.
TranS = readdlm(".//inputs//csv//LineSusceptance.csv", ','; header = true); # Tranmission line Suscopetance, which is not used in this version of the model
Reserve_Reqs = readdlm(".//inputs//csv//data_reserve_reqs.csv", ','; header = true); # Static Reserve target enforced every hour of the year

FuelPrice = readdlm(".//inputs//csv//data_fuel_price.csv", ','; header = true); # primary fuel price for slow-start generators

FuelPrice_head = FuelPrice[2];
FuelPrice = FuelPrice[1];
FuelPrice = FuelPrice[2:N_Gens+1, 4:368];

FuelPricePeakers = readdlm(".//inputs//csv//data_fuel_price_peakers.csv", ','; header = true); # primary fuel price for fast-start generators
...
@time begin
# CSV file for slow-start generatpors outputs in the 6am UC
...
# CSV file for peakers outputs in the 6 am UC
...
# CSV file for storage outputs in 6 am UC
...
# CSV file for transmission flows after 6 am UC
FUCR_TranFlowOutputs_header = ["Day", "Time period", "Source", "Sink", "Flow", "TransCap"]
# CSV file for constraint violations (demand or renewable generation curtailment, over-generation) after 6 am UC
FUCR_Curtail_header         = ["Day", "Hour", "Zone", "OverGeneration", "DemandCurtailment", "WindCrtailment", "SolarCurtailment", "HydroSpillage"]
# csv files for the outputs after 5 pm UC
...
# csv  files for reporting the real-time outputs (BAUC outputs)
...
#FUCRtoBUCR1 represents auxililary variables that tranfer the FUCR's optimal outcomes (6am UC outcomes) to the balancing UC model between 6 am and 5 pm
...
#SUCRtoBUCR2 represents auxililary variables that tranfer the SUCR's optimal outcomes (5pm UC outcomes) to the second balancing UC model that runs between 5 pm to 6am of the next day
...
# the following auxilary varioables are valued in a postprocessing module that determines if a unit has remained online (or offline) enough to meet its min up time or (min down time)
...
# Initial values in FUCR (6am UC) that come from BUCR2
...
# Initial values in SUCR (5pm UC) that come from BUCR1
...
# Initial values in BUCR1 that for the 6 am run they come from BUCR2 (at 5 am ) and for the 7am to 4 pm runs they come from Previous hour BUCR1 run
...
# Initial values in BUCR2 that for the 5 pm run they come from BUCR1 (at 4 pm ) and for the 6pm to 5 am runs they come from Previous hour BUCR2 run
...
for day = INITIAL_DAY:FINAL_DAY
    ...
    # the following block specfies the range of data in csv files (the first and last cell) for the period that we are simulating the 6 am UC (FUCR)
    ...
    # the following block specfies the range of data in csv files (the first and last cell) for the period that we are simulating the 5 pm UC (SUCR)
...
# Declaring the decision variables for conventional slow-start generators
    @variable(FUCRmodel, FUCR_genOnOff[1:N_Gens, 0:N_Hrs_FUCR], Bin) #on/off startus for slow-start generators
    @variable(FUCRmodel, FUCR_genStartUp[1:N_Gens, 1:N_Hrs_FUCR], Bin) # startup variable for slow-start generators
    @variable(FUCRmodel, FUCR_genShutDown[1:N_Gens, 1:N_Hrs_FUCR], Bin) # shutdown variable for slow-start generators
    @variable(FUCRmodel, FUCR_genOut[1:N_Gens, 0:N_Hrs_FUCR]>=0) # Generator's output schedule for slow-start generators
    ...
    @variable(FUCRmodel, FUCR_genResNonSpin[1:N_Gens, 1:N_Hrs_FUCR]>=0) # Scheduled up reserve
    @variable(FUCRmodel, FUCR_genResDn[1:N_Gens, 1:N_Hrs_FUCR]>=0) # Generator's down rserve schedule
    @variable(FUCRmodel, FUCR_TotGenVioP[g=1:N_Gens, 1:N_Hrs_FUCR]>=0) # auxilary decision variables for modeling positive violation of total generation equality
    @variable(FUCRmodel, FUCR_TotGenVioN[g=1:N_Gens, 1:N_Hrs_FUCR]>=0) # auxilary decision variables for modeling negative violation of total generation equality
    @variable(FUCRmodel, FUCR_MaxGenVioP[g=1:N_Gens, 1:N_Hrs_FUCR]>=0) # auxilary decision variables for modeling  violation in maximum generation limit constraints
    @variable(FUCRmodel, FUCR_MinGenVioP[g=1:N_Gens, 1:N_Hrs_FUCR]>=0) # auxilary decision variables for modeling  violation in minimum generation limit constraints
...
# declaring decision variables for renewable generation
    @variable(FUCRmodel, FUCR_solarG[1:N_Zones, 1:N_Hrs_FUCR]>=0) # scheduled solar energy
    @variable(FUCRmodel, FUCR_windG[1:N_Zones, 1:N_Hrs_FUCR]>=0) # scheduled wind energy
    @variable(FUCRmodel, FUCR_hydroG[1:N_Zones, 1:N_Hrs_FUCR]>=0) # scheduled  hydro energy
    @variable(FUCRmodel, FUCR_solarGSpil[1:N_Zones, 1:N_Hrs_FUCR]>=0) # solar energy curtailment/spillage schedules
    @variable(FUCRmodel, FUCR_windGSpil[1:N_Zones, 1:N_Hrs_FUCR]>=0) # wind energy curtailment/spillage schedules
    @variable(FUCRmodel, FUCR_hydroGSpil[1:N_Zones, 1:N_Hrs_FUCR]>=0) # hydro energy curtailment/spillage schedules
...
# Constraints representing technical limits of conventional slow-start generators
...
## This block models the Balancing Unit Commitment Runs between the morning and evening UC Runs
    for h=1:INITIAL_HR_SUCR-INITIAL_HR_FUCR # number of BUCR periods in between FUCR and SUCR
        ...
        # declaring decision variables for renewable generation
        @variable(BUCR1model, BUCR1_solarG[1:N_Zones]>=0) # generated solar energy
        @variable(BUCR1model, BUCR1_windG[1:N_Zones]>=0) # generated wind energy
        @variable(BUCR1model, BUCR1_hydroG[1:N_Zones]>=0) # generated hydro energy
        @variable(BUCR1model, BUCR1_solarGSpil[1:N_Zones]>=0) # solar energy curtailoment
        @variable(BUCR1model, BUCR1_windGSpil[1:N_Zones]>=0) # wind energy curtailment
        @variable(BUCR1model, BUCR1_hydroGSpil[1:N_Zones]>=0) # hydro energy curtailment
